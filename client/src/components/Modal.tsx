        ...
        {isOpen && <div
            ...
            <div className="px-10 py-8 flex flex-col gap-4 rounded-3xl bg-neutral-900 border border-zinc-700" onClick={(e) => e.stopPropagation()}>
                <div className="flex justify-between items-center relative">
                    ...
                    >
                        {!cancelHover ?
                            <ModalSvgButton onClose={onClose} color="bg-blue-400" key={cancelHover ? 1 : 0} Svg={<Utensils size={20} color='#ADD8E6' />} />
                            :
...
export const ModalSvgButton: React.FC<ModalSvgButtonProps> = ({ Svg, color, onClose }) => {
    return (<div
        onClick={onClose}
        className='absolute hover:scale-105 ease-linear duration-75 active:scale-75 top-0 w-fit p-2 translate-x-[-8px] translate-y-[-8px] aspect-square rounded-full  flex justify-center items-center left-0 z-20 overflow-hidden'>
